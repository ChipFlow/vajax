...
This script:
1. Converts ngspice netlists to JAX-SPICE format using netlist_converter
...
def convert_netlist(test_name: str) -> bool:
    """Convert ngspice netlist to JAX-SPICE format."""
    ...
    print(f"Converting {spice_file.name} to JAX-SPICE format...")
    ...
    try:
        # Import the converter from jax_spice
        from jax_spice.netlist_converter.ng2vclib import dfl
        from jax_spice.netlist_converter.ng2vclib.converter import Converter

        # Build configuration
        cfg = dfl.default_config()
        cfg["sourcepath"] = [".", str(ihp_models)]
        cfg["read_depth"] = None
        cfg["process_depth"] = None
        cfg["output_depth"] = None
        cfg["all_models"] = False

        # Create converter with ngspice dialect
        converter = Converter(cfg, dialect="ngspice")
        converter.convert(tmp_path, str(sim_file))
...
def run_simulation(test_name: str) -> bool:
    ...
    vacask = None
    for candidate in vacask_candidates:
        if candidate.exists():
            vacask = candidate
            break

    if not vacask:
        print("Error: VACASK executable not found")
        print("Set VACASK_BIN environment variable to the vacask binary path")
        return False
