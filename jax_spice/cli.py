"""JAX-SPICE command-line interface.

Provides ngspice-style CLI for circuit simulation:
    jax-spice circuit.sim                    # Run with .control section
    jax-spice circuit.sim -o results.raw     # Specify output file
    jax-spice circuit.sim --tran 1n 100u     # Override transient params
    jax-spice benchmark ring                 # Run benchmark
    jax-spice convert input.sp output.sim    # SPICE to VACASK conversion
"""
...
# Configure JAX before importing jax_spice
from jax_spice import CircuitEngine, configure_precision, get_precision_info
...
def cmd_benchmark(args: argparse.Namespace) -> int:
    ...
    from jax_spice.benchmarks.registry import get_benchmark, list_benchmarks
...
def cmd_info(args: argparse.Namespace) -> int:
    ...
    print("JAX-SPICE System Information")
    print("-" * 40)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  jax-spice circuit.sim                    Run with defaults
  jax-spice circuit.sim -o out.raw         Specify output file
  jax-spice circuit.sim --tran 1n 100u     Override transient params
  jax-spice circuit.sim --sparse --gpu     Use sparse solver on GPU
  jax-spice benchmark ring --profile       Run and profile benchmark
  jax-spice convert input.sp output.sim    Convert SPICE to VACASK
        """,
    )
