...
def write_rawfile(
    ...
    n_points = len(x_data)
    1 + len(voltages)  # x variable + all node voltages
    ...
    if binary:
        _write_binary_rawfile(output_path, analysis_type, variables, x_data, voltages, n_points)
    else:
        _write_ascii_rawfile(output_path, analysis_type, variables, x_data, voltages, n_points)
