...
class GPUProfiler:
    ...
    def run_benchmark(
        ...
        try:
            ...
            logger.info("")
            logger.info("      === VACASK-comparable metrics ===")
...
def run_benchmark_subprocess(
    ...
    try:
        ...
        proc.kill()
        logger.error("  Benchmark timed out after 30 minutes")
        return BenchmarkResult(
            name=name,
            nodes=0,
            devices=0,
        ...
        for name, sim_path in benchmarks:
            ...
            if run_dense:
                ...
                if result_dense:
                    ...
                    logger.info("    dense:  FAILED (subprocess error)")
            ...
            if run_sparse:
                ...
                if result_sparse:
                    ...
                    logger.info("    sparse: FAILED (subprocess error)")
