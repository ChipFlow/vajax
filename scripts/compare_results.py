...
def run_vacask_short_tran(sim_path: Path, vacask_bin: Path, steps: int = 5) -> dict:
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".sim", delete=False, dir=sim_path.parent.resolve()
    ) as f:
        f.write(content)
...
def run_jaxspice(sim_path: Path, steps: int = 5) -> dict:
    ...
    engine.prepare(t_stop=stop, dt=step)
...
def compare_benchmarks():
    ...
    for name, config in benchmarks.items():
        ...
        for node in config["nodes_to_compare"]:
            ...
            jax_v = (
                jax_voltages.get(node)
                or jax_voltages.get(node.lower())
                or jax_voltages.get(node.upper())
            )
            vac_v = (
                vacask_voltages.get(node)
                or vacask_voltages.get(node.lower())
                or vacask_voltages.get(node.upper())
            )
            ...
            if jax_v is not None and vac_v is not None:
                ...
                print(
                    f"  {node:<15} {jax_v:>12.6f} {vac_v:>12.6f} {diff:>+12.2e} {rel_err:>9.2%} {status}"
                )
