...
class TestSolutionHistory:
    """Test SolutionHistory circular buffer."""

    def test_push_and_depth(self):
        """Test pushing solutions and checking depth."""
        from jax_spice.analysis.transient.adaptive import SolutionHistory

        history = SolutionHistory(max_depth=3)
        assert history.depth == 0

        # Push first solution
        V1 = jnp.array([1.0])
        Q1 = jnp.array([1e-12])
        history.push(V1, Q1, dt_new=1e-9)
        assert history.depth == 1

        # Push second
        V2 = jnp.array([2.0])
        Q2 = jnp.array([2e-12])
        history.push(V2, Q2, dt_new=1e-9)
        assert history.depth == 2

        # Most recent should be first
        np.testing.assert_allclose(history.V[0], V2)
        np.testing.assert_allclose(history.V[1], V1)

    def test_max_depth_eviction(self):
        """Test that old solutions are evicted when max_depth is reached."""
        from jax_spice.analysis.transient.adaptive import SolutionHistory

        history = SolutionHistory(max_depth=2)

        for i in range(5):
            V = jnp.array([float(i)])
            Q = jnp.array([float(i) * 1e-12])
            history.push(V, Q, dt_new=1e-9)

        # Should only have last 2
        assert history.depth == 2
        np.testing.assert_allclose(history.V[0], jnp.array([4.0]))
        np.testing.assert_allclose(history.V[1], jnp.array([3.0]))

    def test_clear(self):
        """Test clearing history."""
        from jax_spice.analysis.transient.adaptive import SolutionHistory

        history = SolutionHistory(max_depth=4)
        history.push(jnp.array([1.0]), jnp.array([1e-12]), 1e-9)
        history.push(jnp.array([2.0]), jnp.array([2e-12]), 1e-9)
